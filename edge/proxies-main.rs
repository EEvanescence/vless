use anyhow::{Context, Result}; use clap::Parser; use serde::Deserialize; use std::collections::BTreeMap; use std::fs::{self, File}; use std::io::{self, BufRead, BufReader, Write}; use std::path::Path; use std::sync::{Arc, Mutex}; use std::time::{Duration, Instant}; use chrono::{Duration as ChronoDuration, Utc}; use chrono_tz::Asia::Tehran; use colored::*; use futures::StreamExt; use reqwest::{Client};

const DEFAULT_PROXY_FILE: &str = "edge/assets/p-list-november.txt"; const DEFAULT_OUTPUT_FILE: &str = "sub/ProxyIP-Daily.md"; const DEFAULT_MAX_CONCURRENT: usize = 60; const DEFAULT_TIMEOUT_SECONDS: u64 = 6; const REQUEST_DELAY_MS: u64 = 40;

const GOOD_ISPS: &[&str] = &[ "M247", "Vultr", "GCore", "IONOS", "Google", "Amazon", "NetLab", "Akamai", "Turunc", "Contabo", "UpCloud", "Tencent", "Hetzner", "Multacom", "HostPapa", "Ultahost", "DataCamp", "Bluehost", "Scaleway", "DO Space", "Leaseweb", "Hostinger", "Hypercore", "ByteDance", "RackSpace", "SiteGround", "Online Ltd", "The Empire", "Cloudflare", "Relink LTD", "PQ Hosting", "Gigahost AS", "White Label", "G-Core Labs", "3HCLOUD LLC", "HOSTKEY B.V", "DigitalOcean", "3NT SOLUTION", "Zenlayer Inc", "RackNerd LLC", "Plant Holding", "WorkTitans BV", "IROKO Networks", "WorldStream B.V", "Cluster Logic Inc", "The Constant Company", "Cogent Communications", "Metropolis networks inc", "Total Uptime Technologies", ];

#[derive(Parser, Clone)] #[command(name = "Proxy Checker")] #[command(about = "Checks proxies via Cloudflare /meta and outputs active ones")] struct Args { #[arg(short, long, default_value = DEFAULT_PROXY_FILE)] proxy_file: String,
...
#[derive(Debug, Clone, Deserialize)] struct CfMeta { #[serde(rename = "clientIp")] client_ip: String,
...
#[derive(Debug, Clone)] struct ProxyInfo { ip: String, isp: String, country: String, region: String, city: String, }
...
async fn fetch_cf_meta(proxy: Option<(String, u16)>) -> Result<CfMeta> { let host = "speed.cloudflare.com"; let url = format!("https://{}/meta", host);
...
async fn process_proxy( proxy_line: String, active_proxies: &Arc<Mutex<BTreeMap<String, Vec<(ProxyInfo, u128)>>>>, self_ip: &str, ) { let parts: Vec<&str> = proxy_line.split(',').collect(); if parts.len() < 2 { return; } let ip = parts[0]; let port = parts[1].parse::<u16>().unwrap_or(443);
...
fn write_markdown_file(proxies_by_country: &BTreeMap<String, Vec<(ProxyInfo, u128)>>, output_file: &str) -> io::Result<()> { let mut file = File::create(output_file)?;
...
writeln!(file, "  <img src=\"https://img.shields.io/badge/Countries-{}-yellow\" />", countries_badge_label)?;
writeln!(file, "  <img src=\"https://img.shields.io/badge/Avg_Latency-{}-orange\" />", latency_badge_label)?;
writeln!(file, "</p>\n")?;
...
> <p><b>Daily Fresh Proxies</b></p>A curated list of <b>high-quality</b>, fully-tested proxies sourced from reputable ISPs and major global data centers (e.g., Google, Amazon, Cloudflare, Tencent, Hetzner, and others).

<br/><p><b>Auto-Updated Daily</b></p>Last update: {} (UTC+3:30)
Next update: {} (UTC+3:30)

<br/><p><b>Overview</b></p>Active proxies: {}
Countries covered: {}
Average latency: {} ms
...
"##, last_updated_str, next_update_str, total_active, total_countries, avg_ping )?;
...
fn country_flag(code: &str) -> String { code.chars() .filter_map(|c| { if c.is_ascii_alphabetic() { Some(char::from_u32(0x1F1E6 + (c.to_ascii_uppercase() as u32 - 'A' as u32)).unwrap()) } else { None } }) .collect() }

fn read_proxy_file(file_path: &str) -> io::Result<Vec<String>> { let file = File::open(file_path)?; let reader = BufReader::new(file); let mut proxies = Vec::new();
