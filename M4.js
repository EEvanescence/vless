 ...
 * - Last Update: Tuesday, Feb 26, 2026, 04:20 UTC
 ...
 *    - UUID in line 31.
 *    - Hostname in each group's "host" parameter (lines 54, 67, 80).
 *    - SNI in Each group's "sni" parameter (lines 55, 81).
 ...
 *
 * FEATURES:
 * - Generates 30 VLESS configs (10 per group by default, customizable via ipCount line 34.)
 ...
 * - Randomizable paths, fingerprints and IPs for better censorship resistance.
 *
 */
...
const USER_SETTINGS = {
   ...
   * - Each group can have different settings for hosts, ports, TLS, IP source, fp, alpn and etc.
   *
  ...
  groups: [
    {
      ...
      path: "/random:26", // Path with 16 random characters
      tls: true,
      allowInsecure: true,
...
// EVENT LISTENER (For Old Worker Format)
...
async function handleRequest(_request) {
  try {
    ...
    // Fetch dynamic IP lists from external sources with timeout to prevent 1101 crashes due to hanging APIs
    ...
    for (const group of USER_SETTINGS.groups) {
      ...
      for (const ip of ipList) {
        if (seen.size >= USER_SETTINGS.ipCount) break;
        if (!seen.has(ip)) {
          configsList.push(createVlessLink(ip, group, USER_SETTINGS));
          seen.add(ip);
        }
      }
    }

    if (configsList.length === 0) throw new Error("No configs generated");
    ...
    const headers = {
      ...
      "Profile-Title": encodeURIComponent(profileTitle),
    };

    return new Response(toBase64(configsList.join("\n")), { status: 200, headers });
  } catch (error) {
    // Error handling - return empty config list on failure
    return new Response(toBase64("# Error generating configurations\n# Reason: " + error.message), {
      status: 200,
...
function createVlessLink(ip, group, settings) {
  ...
  if (finalPath.includes("random:")) {
    try {
      const len = parseInt(finalPath.match(/random:(\d+)/)?.[1] || "10");
      finalPath = finalPath.replace(/random:\d+/, generateRandomPath(len));
    } catch (e) {} // On error, keep original path
  }
  ...
  if (group.tls) {
    ...
    queryParams.set("sni", group.sni || group.host);
    if (group.alpn) queryParams.set("alpn", group.alpn);
    if (group.allowInsecure) queryParams.set("allowInsecure", "1");
  }

  return `vless://${settings.uuid}@${ip}:${randomPort}?${queryParams.toString()}#${encodeURIComponent(group.name)}`;
}
...
function generateCakeSubscriptionInfo() {
  const GB = 1024 ** 3;
  const TB = 1024 * GB;
  const total = CAKE_INFO.total_TB * TB;
  const base = CAKE_INFO.base_GB * GB;

  // Calculate dynamic usage based on current hour of day
  const now = new Date();
  const hoursNow = now.getHours() + now.getMinutes() / 60;
  const growth = (hoursNow / 24) * (CAKE_INFO.daily_growth_GB * GB);
  const used = base + growth;

  // Convert expiry date to Unix timestamp
  const expire = Math.floor(new Date(CAKE_INFO.expire_date).getTime() / 1000);
